...
class Function():
    ...
    def set_function(self, cpu, memory):
        ...
        memory_duration = (self.params.max_duration - self.params.min_duration) * 1 / (self.params.k + 1)
        if self.memory >= self.params.ideal_memory:
            memory_delay_factor = 0
        else:
            memory_delay_factor = (self.params.ideal_memory - self.memory) / (self.params.ideal_memory - self.params.memory_least_hint)
        memory_duration_increment = memory_duration * memory_delay_factor

        self.duration = int(self.params.min_duration + cpu_duration_increment + memory_duration_increment) + 1

        # print("ideal_cpu: {}, cpu: {}".format(self.params.ideal_cpu, self.cpu))
...
class RequestRecord():
    ...
    def update_requests(self, done_request_list):
        ...
        for request in done_request_list:
            ...
            slo = request.get_duration_slo()
            if slo < 1:
                good_slo = good_slo + (1 - slo)
            elif slo > 1:
                # bad_slo = bad_slo + (slo - 1) * (request.get_cpu_user_defined() + request.get_memory_user_defined())
                bad_slo = bad_slo + (slo - 1)

            total_slo = total_slo + slo
        ...
        return good_slo, bad_slo, total_slo
