...
class Environment():
    ...
    def __init__(
        ...
        self.profile, self.event_pq = self.workload_generator.generate_workload(incremental=incremental)
    ...
    def get_observation(
        ...
        state.append(function_avg_duration)
        state.append(function_baseline)
        state.append(function.get_cpu())
        state.append(function.get_memory())
        ...
        if len(last_request) == 0:
            ...
            if last_cpu_peak / cpu_user_defined <= params.SAFEGUARD_THRESHOLD: # Over-provisioned
                if last_cpu_peak / last_cpu_alloc >= params.SAFEGUARD_THRESHOLD: # Usage spike
                    cpu_range = [cpu_user_defined]
                    is_safeguard = True
            ...
            if last_mem_peak / memory_user_defined <= params.SAFEGUARD_THRESHOLD: # Over-provisioned
                if last_mem_peak / last_mem_alloc >= params.SAFEGUARD_THRESHOLD: # Usage spike
                    memory_range = [memory_user_defined]
                    is_safeguard = True
    ...
    def get_reward(
        ...
        return -total_duration_slo + good_slo - bad_slo
